...
class Twitterutilities {
  ...
  public static function entify($tweet, &$replacements=array()) {
    return TwitterUtilities::entify_with_options($tweet, array(), $replacements);
  }
   ...
   */
  public static function auto_fix_time_request($TwitterOAuth, $method, $url, $params=array(), $useauth=true, $multipart=false) {
    $TwitterOAuth->request($method, $url, $params, $useauth, $multipart);
    ...
    if ($TwitterOAuth->response['code'] != 401)
      return;
    ...
    if (stripos($TwitterOAuth->response['response'], 'password') !== false)
     return;

    // force the timestamp to be the same as the Twitter servers, and re-request
    $TwitterOAuth->auto_fixed_time = true;
    $TwitterOAuth->config['force_timestamp'] = true;
    $TwitterOAuth->config['timestamp'] = strtotime($TwitterOAuth->response['headers']['date']);
    return $TwitterOAuth->request($method, $url, $params, $useauth, $multipart);
  }
